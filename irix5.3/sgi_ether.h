...
/*
 * =====================================================================
 * IRIX 5.3 copy of SGI's private kernel <ether.h>.
 *
 * READ THIS BEFORE FIRST BOOT.
 *
 * ether.h is NOT shipped in /usr/include - it is an internal kernel
 * header, so this file is a reconstruction.  It was derived from the
 * 6.5 copy in ../sgi_ether.h ($Revision: 1.15 $) by removing the INET6
 * member, which did not exist in 1994.
 *
 * struct etherif is embedded by value as the first member of struct
 * dp_softc, and ether_attach() writes through it.  If the real 5.3
 * kernel's struct etherif has a DIFFERENT member list from this file,
 * ether_attach() will write outside the fields we think it will and
 * corrupt adjacent softc memory.  That failure is silent - there is no
 * link error and no compile warning.
 *
 * Most of the size is safe automatically: eif_arpcom, eif_mfilter and
 * eif_rawif come from 5.3's own shipped net/if.h, net/multi.h and
 * net/raw.h, so they pick up the correct 5.3 layouts when compiled on
 * the target.  The residual risk is confined to etherif's OWN member
 * list - principally whether eif_resets, eif_lostintrs and eif_sick
 * exist in 5.3 and in this order.
 *
 * Verify on the target before trusting this file - see the
 * "Verifying struct etherif" section of README.md.
 * =====================================================================
 */
#define ETHER_H
/*
...
struct etherif {
	...
	/*
	 * The 6.5 header has an "#ifdef INET6 struct in6_addr eif_llip6"
	 * member here.  IRIX 5.3 has no INET6, so it is removed rather
	 * than left behind an #ifdef that could never be true.
	 */
};
